<template>
    <form @submit.prevent="addItem" class="card card-body mb-3" accept="image/jpg, image/jpeg">
        <div v-for="item in this.data.fields" v-bind:key="item.name" style="width: 18rem;">
            <input type="text" v-bind:placeholder="item.name" v-bind:name="item.fieldName" v-bind:id="item.name" v-bind:type="item.type" v-model="data.item[item.name]">
        </div>
    ...
    methods: {
      parsePropsToState(arr){
        let newArr = arr.map((el, idx) => {
          ...
          this.data.item[el] = '';
          return {fieldName: el.split('_').join(" "), type: el==='photo'? 'file': 'string', name: el}
